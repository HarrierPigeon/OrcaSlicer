...
void GCode::_do_export(Print& print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    if(!has_BTT_thumbnail){   
        ...
        // Belt printer: embed angle and Z offset in header for G-code processor detection.
        if (print.config().belt_printer.value) {
            file.write_format("; belt_printer_angle = %.1f\n", print.config().belt_printer_angle.value);
            // Use max Z offset across all objects for the preview transform.
            double max_z_off = 0.;
            for (const PrintObject *obj : print.objects())
                max_z_off = std::max(max_z_off, obj->belt_z_offset());
            file.write_format("; belt_z_offset = %.4f\n", max_z_off);
        }
        if (is_bbl_printers)
    ...
    for (unsigned int extruder_id : layer_tools.extruders)
    {
        ...
        for (int print_wipe_extrusions = is_anything_overridden; print_wipe_extrusions>=0; --print_wipe_extrusions) {
            ...
            for (InstanceToPrint &instance_to_print : instances_to_print) {
                ...
                // Belt printer: update per-object Z offset on the writer.
                if (m_writer.is_belt_printer())
                    m_writer.set_belt_z_offset(instance_to_print.print_object.belt_z_offset());
