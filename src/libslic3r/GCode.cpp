...
void GCode::_do_export(Print& print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    // Belt printer: create a BeltGCodeWriter with coordinate transformation and axis remap.
    if (print.config().belt_printer.value) {
        ...
        belt_writer->set_build_volume_max(Vec3d(bbox_bed.max.x(), bbox_bed.max.y(), print.config().printable_height.value));
...
void GCode::update_origin_snap(const PrintObject *obj, const Point &inst_shift)
{
    ...
    // Origin snap is only active for belt printers, so m_writer is a BeltGCodeWriter.
