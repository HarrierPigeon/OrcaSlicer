...
public:
    ...
    void set_belt_z_offset(double z_offset) { m_belt_z_offset = z_offset; }
    bool is_belt_printer() const { return m_belt_angle_rad != 0.; }
    // Transform a point from the rotated (slicing) frame to world/machine coordinates.
  ...
  private:
    ...
    double          m_belt_z_offset = 0.;  // Z translation applied after rotation in slicing
