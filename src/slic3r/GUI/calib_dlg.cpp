...
// ORCA-Belt: PA Line / PA Pattern generate raw bed-plane G-code that bypasses
// the belt coordinate pipeline, so belt printers only support the PA Tower.
...
void PA_Calibration_Dlg::on_method_changed(wxCommandEvent& event) {
    // ORCA-Belt: only the sliced PA Tower method is supported on belt printers.
...
void PA_Calibration_Dlg::on_show(wxShowEvent& event) {
    ...
    if (is_belt_printer_selected()) {
        ...
        const wxString tip = _L("Not available on belt printers: this method generates raw bed-plane G-code. Use the PA Tower method instead.");
